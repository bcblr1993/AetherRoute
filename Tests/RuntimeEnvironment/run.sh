...
TEST_TEMP=$(mktemp -d "${TMPDIR:-/tmp}/aetherroute-runtime-environment.XXXXXX")
...
swiftc -swift-version 6 -warnings-as-errors -parse-as-library \
  ...
  "$ROOT/Sources/AetherRouteApp/AppRuntimeEnvironmentController.swift" \
  "$ROOT/Tests/RuntimeEnvironment/main.swift" \
  -o "$TEST_TEMP/runtime-environment"
"$TEST_TEMP/runtime-environment"
