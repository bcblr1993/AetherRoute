...
4. **Feed synchronization**:
   - Commit the generated `appcast.xml` to `main` and tag the release, so the signature for this version is available at the tag.
   - The live update feed is `https://aethernative.com/apps/aetherroute/appcast.xml`, generated by the official website from its release records.
5. **Publish to aethernative.com**:
   - Publish the GitHub Release (attach the notarized DMG). The `aethernative-sync` workflow notifies the website, which imports the version, SHA-256 (from the asset digest) and Sparkle signature (from the release body block, or `appcast.xml` at the tag), verifies the signature against `SUPublicEDKey`, and redeploys.
   - Add the user-facing notes as a hidden block at the end of the release body — engineering details elsewhere in the body are never published:
     ```
     <!-- aethernative
     summary:
       zh: 一句话中文说明
     ...
     -->
     ```
   - Editing the release later re-syncs it. Check progress at https://github.com/bcblr1993/aethernative-site/actions.
