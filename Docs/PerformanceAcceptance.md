...
| Connected idle | Combined host, extension, and core median CPU at or below 2%, p95 at or below 5%, combined resident memory at or below 256 MiB | Signed TUN and Transparent Proxy ten-minute samples | Pending signing inputs |
| Core lifecycle | FlowOnly peak RSS at or below 64 MiB and PacketFlow at or below 32 MiB; no leaked processes or file descriptors | Verifier-backed 500-cycle gates and source manifest | Current `4b03bf8` artifacts pass the bounded functional lifecycle gates: FlowOnly completes 500 staged cycles across two engines with three loopback flows and six callbacks, while PacketFlow completes 500 startup/selector/telemetry/shutdown cycles at 12,468,224-byte peak RSS. Final numeric release evidence still requires the exact frozen-source schema-2 soak; prior long-run measurements are revision-bound |
...
| Large import | A 5,000-node fixture imports in at most two seconds with at most 100 MiB temporary RSS growth; cancellation remains responsive | Release-build XCTest metric and memory trace | Passing: the latest current arm64 Release gate reads, validates, encrypts, and activates 5,000 nodes in 0.110973 seconds with 3,424,256 bytes of sampled peak RSS growth. A controlled blocked-reader test proves cancellation returns `CancellationError` and leaves the encrypted catalog empty; line-scanning validation also has bounded cancellation checkpoints |
| TCP throughput | Current arm64 core throughput at least 1,024 MiB/s for each engine; p95 added loopback latency at or below 5 ms; raw kernel-direct ratio and later release-to-release regression remain visible | Same-host baseline and engine run, five 32 MiB repetitions, 200 latency samples, current artifact and harness hashes | No passing current-core result is retained. The latest run was rejected before evaluating either engine because its direct controls reached only 1,125.057 and 1,234.235 MiB/s, below the mandatory 2,048 MiB/s measurement baseline while the host was under load and a third-party proxy extension remained active. Historical 3,612.962/3,127.443 MiB/s engine results belong to an earlier artifact and are regression context only. Rerun on a quiet clean M-series Mac; a signed Network Extension end-to-end result also remains pending signing inputs |
| UDP integrity | Zero missing or duplicated payloads across 10,000 numbered loopback datagrams after bounded warm-up | FlowOnly and PacketFlow black-box gate | Passing on both current arm64 core artifacts: after 32 warm-up payloads, FlowOnly and PacketFlow each delivered 10,000/10,000 numbered payloads with zero missing or duplicates and 9,093,120/11,747,328-byte peak RSS. PacketFlow uses a one-packet acknowledged integrity window; burst throughput is measured by the separate throughput gate |
...
| Sleep and path change | No crash or UI freeze; state becomes truthful within ten seconds and reconnect remains cancellable | Signed sleep/wake, Wi-Fi path-change, and recovery matrix | Pending signing inputs |
