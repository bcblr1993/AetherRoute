...
usage() {
  echo "usage: $0 user@host /absolute/notarized-candidate-directory unique-release-id" >&2
}

test -n "$HOST" && test -n "$CANDIDATE_DIRECTORY" && test -n "$RELEASE_ID" || {
  usage
  exit 64
}
...
test -d "$CANDIDATE_DIRECTORY" || {
  echo "candidate directory does not exist" >&2
  exit 1
}
...
test -f "$DMG" && test -f "$SUMS" || {
  echo "candidate directory does not contain the expected notarized preview" >&2
  exit 1
}
...
mkdir -p "$temporary/payload/site" "$temporary/payload/downloads/prerelease" "$temporary/payload/updates"
...
cp "$CANDIDATE_DIRECTORY/README.txt" "$temporary/payload/downloads/prerelease/README.txt"
...
echo "Deployed AetherRoute web release: $RELEASE_ID"
