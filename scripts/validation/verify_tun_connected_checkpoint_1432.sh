...
usage() {
  echo "usage: $0 evidence expected-build expected-app-cdhash expected-tun-cdhash expected-tun-sha256 pass|known-fail zero|idle-ok" >&2
  exit 64
}
...
TRANSPARENT_PROCESS_POLICY=$7
case "$EVIDENCE" in /*) ;; *) usage;; esac
case "$EXPECTED_STUN_POLICY" in pass|known-fail) ;; *) usage;; esac
case "$TRANSPARENT_PROCESS_POLICY" in zero|idle-ok) ;; *) usage;; esac
...
test "$(field transparent_process_policy)" = "$TRANSPARENT_PROCESS_POLICY" || fail "transparent process policy mismatch"
...
transparent_pid_count=$(field transparent_pid_count)
case "$transparent_pid_count" in ''|*[!0-9]*) fail "transparent process count invalid";; esac
case "$TRANSPARENT_PROCESS_POLICY" in
  zero) test "$transparent_pid_count" -eq 0 || fail "transparent provider was running";;
  idle-ok) test "$transparent_pid_count" -le 1 || fail "multiple transparent providers were running";;
esac
