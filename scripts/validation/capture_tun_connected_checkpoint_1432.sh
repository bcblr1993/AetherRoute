...
usage() {
  echo "usage: $0 output expected-build expected-app-cdhash expected-tun-cdhash expected-tun-sha256 https-probe https-probe-sha256 stun-probe stun-probe-sha256 pass|known-fail zero|idle-ok" >&2
  exit 64
}
...
case "$TRANSPARENT_PROCESS_POLICY" in
  zero) test "$transparent_pid_count" -eq 0 || baseline=failed;;
  idle-ok) test "$transparent_pid_count" -le 1 || baseline=failed;;
esac
...

{
  ...
  printf 'transparent_process_policy=%s\n' "$TRANSPARENT_PROCESS_POLICY"
  printf 'app_build=%s\n' "$app_build"
  printf 'app_cdhash=%s\n' "$app_cdhash"
