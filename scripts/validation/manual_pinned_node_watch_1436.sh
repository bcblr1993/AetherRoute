...
REMOTE
    ...
    cp "$0" "$OUTPUT/manual_pinned_node_watch_1436.sh"
    chmod 400 "$OUTPUT/manual_pinned_node_watch_1436.sh"
...
REMOTE
    ...
    A_INBOUND=$(grep -c 'inbound connection from 192\.168\.64\.6' "$OUTPUT/fixture-a-delta.log" || true)
    B_INBOUND=$(grep -c 'inbound connection from 192\.168\.64\.6' "$OUTPUT/fixture-b-delta.log" || true)
