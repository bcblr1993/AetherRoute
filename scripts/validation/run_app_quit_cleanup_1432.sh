...
EXPECTED_TUN_SHA256=906f992f210236104449aeec5a4023c2fac4b4663809943a269dd75cef2a5287

mkdir -m 700 "$OUTPUT"
ditto "$0" "$OUTPUT/run_app_quit_cleanup_1432.sh"
chmod 400 "$OUTPUT/run_app_quit_cleanup_1432.sh"
...
i=0
while pgrep -x "$TUN_PROVIDER" >/dev/null 2>&1; do
  i=$((i + 1))
  test "$i" -le 30
  sleep 1
done
tun_pid_after_quit=$(pgrep -x "$TUN_PROVIDER" 2>/dev/null || true)
test -z "$tun_pid_after_quit"
...
test -z "$(pgrep -x "$TUN_PROVIDER" 2>/dev/null || true)"
...

{
  ...
  printf 'tun_pid_after_quit=absent\n'
  printf 'tun_pid_after_relaunch=absent\n'
  printf 'tun_cdhash=%s\n' "$tun_cdhash"
  printf 'tun_sha256=%s\n' "$tun_sha"
...
echo "TUN app-quit cleanup passed: old_app=$app_pid new_app=$new_app_pid old_tun=$tun_pid"
