...
SING_BOX_BIN=${SING_BOX_BIN:?set SING_BOX_BIN to sing-box 1.13.15 darwin arm64}
SHADOW_TLS_BIN=${SHADOW_TLS_BIN:?set SHADOW_TLS_BIN to shadow-tls 0.2.25 darwin arm64}
...
PROFILE_FLAG=
if [ "${AETHER_INTEROP_RELEASE:-0}" = 1 ]; then
  PROFILE_FLAG=--release
fi
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/aetherroute-interop.XXXXXX")
SERVER_PID=
SHADOW_TLS_PID=
...
sed \
  ...
  "$ROOT/Tests/Interop/sing-box-server.json.template" \
...
run_matrix_with_diagnostics() {
  cycle=$1
  if ! run_matrix; then
    echo "sing-box log for failed cycle $cycle:" >&2
    tail -200 "$WORK_DIR/server.log" >&2
    echo "ShadowTLS log for failed cycle $cycle:" >&2
    tail -200 "$WORK_DIR/shadow-tls.log" >&2
    return 1
  fi
}
...
while [ "$cycle" -le "$CYCLES" ]; do
  if [ "$cycle" -eq 1 ]; then
    echo "Interop cycle $cycle: cold start"
  else
    echo "Interop cycle $cycle: server restart"
  fi
  start_server
  run_matrix_with_diagnostics "$cycle"
  stop_server
  cycle=$((cycle + 1))
done

echo "Protocol interoperability and restart gate passed: cycles=$CYCLES"
