...
SMOKE_TEMP=$(mktemp -d "${TMPDIR:-/tmp}/aetherroute-direct-core-smoke.XXXXXX")
cleanup() {
...
AETHER_SMOKE_CYCLES=${AETHER_SMOKE_CYCLES:-500} \
AETHER_SMOKE_RSS_BUDGET_BYTES=${AETHER_SMOKE_RSS_BUDGET_BYTES:-33554432} \
  "$SMOKE_TEMP/core_smoke" \
    "$SMOKE_TEMP/runtime" \
    "$SMOKE_TEMP/ignored.log"

if [ -e "$SMOKE_TEMP/ignored.log" ]; then
  echo "Direct core unexpectedly opened the compatibility log path" >&2
  exit 1
fi

if find "$SMOKE_TEMP/runtime" -type f ! -name existing-sentinel -print -quit \
  | grep -q .; then
  echo "Direct core unexpectedly persisted runtime data during smoke" >&2
  exit 1
fi
